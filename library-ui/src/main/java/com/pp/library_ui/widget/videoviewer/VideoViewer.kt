...
open class VideoViewer : FrameLayout, LifecycleEventObserver {
    ...
    var useBottomTimeBar = true
        set(value) {
            field = value
            updateTimeBar()
        }
    var coverScaleType = ScaleType.CENTER_INSIDE
        set(value) {
            field = value
            videoCoverView.scaleType = field
        }
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
        ...
        val scaleIndex = typedArray.getInt(R.styleable.VideoViewer_android_scaleType,0)
        if (scaleIndex >= 0){
            coverScaleType = sScaleTypeArray[scaleIndex]
        }
        typedArray.recycle()
